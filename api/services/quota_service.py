...
async def _authorize_local_billing(
    *,
    ...
    """Authorize against the local credit ledger and stash rate + affordable seconds.

    The resolved rate is written to the run's ``cost_info`` at authorize time so the

...
async def get_user(
    ...
    try:
        ...
        if user_model.selected_organization_id != organization.id:
            # The first user to land on a freshly created org (Stack "team") is its
            # admin; subsequent joiners default to member.
...
def require_org_role(min_role: Role):
    ...
    Wraps `get_user_with_selected_organization` (which already 400s if the caller
    has no selected org) with a role check against the caller's membership row for
    that org. Superuser bypasses the role check entirely, matching `get_superuser`'s
    "superuser can do anything" model. Auth-mode-agnostic: it never inspects
    `AUTH_PROVIDER`, because it operates purely on the resolved user + membership.
    """
    ...
    async def _dependency(
        ...
        if role_value is None:
            # Should not normally be reachable, but defend against a stale/orphaned

...
async def _debit_local_billing(workflow_run, organization_id: int) -> None:
    """Deduct the run's priced cost from the local credit ledger (idempotent)."""
...
async def report_workflow_run_platform_usage(workflow_run) -> None:
    """Report platform usage for a completed workflow run.

    With the local billing engine active, deduct the run's cost from the local
    credit ledger and return. Otherwise fall through to MPS reporting.
    """
    if BILLING_ENGINE == BILLING_LOCAL:
        ...
        if organization_id is None:
            logger.warning(
                "Skipping local billing for workflow run {}: no organization_id",
                workflow_run.id,
            )
            return

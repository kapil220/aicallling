...
# Org membership. The association row *is* the membership — one role per
# (user, org) pair. Promoted from a bare Table() to a mapped model in Phase 4 so it
# can carry `role`/`created_at`; table name and PK columns are unchanged, so existing
# rows/FKs and all `secondary=organization_users_association` relationships are intact.
...
# (relationship() calls below) and `insert(organization_users_association)` in
# organization_client reference the mapped model's underlying Table.
...
class CreditLedgerModel(Base):
    """Append-only credit ledger. Source of truth for an organization's balance.

    Each row records a signed cents delta and the running balance after it. The
    local billing engine (BILLING_ENGINE=local) writes topups, debits, adjustments,
    and refunds here; the cached OrganizationModel.credit_balance_cents mirrors the
    latest balance_after_cents for fast, row-locked mutation.
    """
    ...
    organization = relationship(
        "OrganizationModel", back_populates="credit_ledger_entries"
    )
...
class PricingRuleModel(Base):
    """Maps a call architecture (mode + provider tuple) to a per-minute rate.

    A null field is a wildcard. Most-specific matching rule wins; ties break on
    higher priority. organization_id null = a global default rule.
    """
...
class PaymentPackModel(Base):
    """Prepaid credit-pack catalog (Phase 3). Read by GET /billing/packs; referenced by
    id when creating a Stripe Checkout Session. Deactivating a pack hides it from the
    catalog without invalidating historical PaymentModel rows that reference it."""
    ...
    __table_args__ = (Index("ix_payment_packs_active", "is_active"),)


class PaymentModel(Base):
    """One row per Stripe Checkout attempt — the Stripe-facing audit trail (Phase 3).
    The credit ledger (Phase 1) stays generic; this table is what "payment history"
    reads from and is the join between a Stripe event and the ledger row it produced."""
